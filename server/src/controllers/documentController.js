...
const documentController = {
  ...
  async search(req, res) {
    try {
      ...
      // В поиске показываем только документы в библиотеке и архиве
      // pending_approval и pending_deletion видны только через страницу Согласование
      if (req.user.role === 'admin') {
        // Админы видят всё кроме pending
        sql += ` AND d.status IN ('in_library', 'archived')`;
      } else {
        // Остальные тоже видят только библиотеку и архив
        sql += ` AND d.status IN ('in_library', 'archived')`;
      }
  ...
  // Просмотр файла (для отображения в браузере)
  ...
  // Создание документа (сразу на согласование)
  async create(req, res) {
    try {
      ...
      // Создаем документ сразу со статусом pending_approval
      const result = await pool.query(`
      ...
      // Записываем согласующих в отдельную таблицу
      ...
      // Автоматически добавляем тег "Новый" или "Новое"
      ...
      // Добавляем выбранные пользователем теги (только валидные id)
      if (tags) {
        const tagIds = typeof tags === 'string' ? JSON.parse(tags) : tags;
        if (Array.isArray(tagIds)) {
          for (const tagId of tagIds) {
            // Проверяем что это валидный id существующего тега
  ...
  // Обновление документа (версия увеличивается только для in_library)
  async update(req, res) {
    try {
      ...
      if (file) {
        ...
        // Увеличиваем версию только если документ уже в библиотеке
        if (isInLibrary) {
      ...
      // Создаем версию только если документ уже в библиотеке и загружен новый файл
      if (file && isInLibrary) {
        await pool.query(`
