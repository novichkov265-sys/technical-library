...
// librarian и admin видят все
  ...
  async addMessage(req, res) {
    try {
      ...
      // Уведомляем автора заявки, если сообщение не от него
  ...
  async resubmit(req, res) {
    ...
    // Обновить документ в тикете (загрузить новый файл)
  async updateDocument(req, res) {
    ...
    try {
      ...
      // Проверяем права - только автор заявки может обновлять документ
      if (ticket.created_by !== userId) {
        await client.query('ROLLBACK');
        const fs = require('fs');
        fs.unlinkSync(req.file.path);
      ...
      // Проверяем статус - можно обновлять только pending или changes_requested
      ...
      // Сохраняем старую версию в document_versions
      ...
      // Если статус был changes_requested, меняем на pending
      if (ticket.status === 'changes_requested') {
        await client.query(
          `UPDATE approval_tickets SET status = 'pending', updated_at = NOW() WHERE id = $1`,
          [id]
